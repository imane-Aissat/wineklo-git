...
class FoodieCubit extends Cubit<Foodie?> {
  ...
  Future<void> updateFoodieDetails({String? fullname, String? email, String? phoneNumber}) async {
    ...
    print("Current foodie state in update prof: $currentFoodie");
  ...
  Future<void> addToFavorites(int restauId) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('jwt_token');
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/favorites/$restauId"),
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
      },
    ...
    if (response.statusCode == 201) {
      ...
      print("Restaurant is already in favorites.");
    } else {
